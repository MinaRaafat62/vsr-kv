...
void advance_to_next_replica(int total_replicas) {
    ...
    std::cout << "\n[FAILOVER] Target " << old_target << " may be down. Advancing to target replica " << new_target << "." << std::endl;
    std::cout << "> " << std::flush;
}
...
void send_message(vsr_message& msg, int total_replicas) {
    ...
    msg.header.request = next_request_number++; // Use the current value, then increment for the next one.
...

    {
        ...
        if (it == clients.end() || !it->second->is_connected()) {
            // The target is known to be disconnected.
        } else {
...
int main() {
    ...
    for (const auto& replica : cluster_config) {
        ...
        client->set_on_disconnect([replica_id, total_replicas](auto conn) {
            ...
            if (current_target_replica_id.load() == replica_id) {
                std::cout << "[FAILOVER] Current target has disconnected. Next command will failover." << std::endl;
            }
            std::cout << "> " << std::flush;
        });
    ...
    // The loop is ending, we can detach the network thread and let the OS clean it up.
